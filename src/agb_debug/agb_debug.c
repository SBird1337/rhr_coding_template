...
/* === PROTOTYPES === */
int dmini_vsnprintf(char * buffer, u32 buffer_len,
    const char * fmt, va_list va);
void dprint(const char * sz);
u32 dmini_strlen(const char * s);
u32 dmini_itoa(int value, u32 radix, u32 uppercase, u32 unsig, char * buffer, u32 zero_pad);
    ...
    const char * fmt, va_list va)
{
    ...
    while ((ch = * (fmt++))) {
        ...
        if (ch != '%')
            ...
            case 'u':
            case 'd':
                len = dmini_itoa(va_arg(va, u32), 10, 0, (ch == 'u'), bf, zero_pad);
                _puts(bf, len);
                break;
            case 'x':
            case 'X':
                len = dmini_itoa(va_arg(va, u32), 16, (ch == 'X'), 1, bf, zero_pad);
                _puts(bf, len);
                break;
            case 'c':
...
void dprintf(const char * str, ...)
{
    ...
    va_list args;
    va_start(args, str);
    dmini_vsnprintf(__outstr, 256, str, args);
    va_end(args);
    dprint(__outstr);
    free(__outstr);
}
